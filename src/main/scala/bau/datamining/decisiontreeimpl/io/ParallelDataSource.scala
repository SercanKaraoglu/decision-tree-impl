...
import bau.datamining.decisiontreeimpl.DataSet
...
object ParallelDataSource {
  def apply(path: String, types: Array[String]): ParallelDataSource = {
    val r = ReadToVector.fromFile(path, types)
...
object ReadToVector {
  def fromFile(path: String, types: Array[String]): (Vector[Line], TemplateMetaData) = {
    val lines = Source.fromFile(new File(path)).getLines()
    val headerText = lines.take(1).next()
    val header = headerText.split(";").map(_.replace("\"", ""))
    val map: Iterator[Line] = lines.map(it => it.split(";").map(_.replace("\"", ""))).map(new Line(_))
    (lines.map(it => it.split(";").map(_.replace("\"", ""))).map(new Line(_)).toVector, TemplateMetaData(types, header))
  }
